/**
 * MCP Server function for Create a player
 */
...
function getPost_PlayersHandler(config: APIConfig): (ctx: any, request: MCPRequest) => Promise<MCPToolResult> {
    ...
    if (args.hideTitle !== undefined) {
        queryParams.push(`hideTitle=${args.hideTitle}`);
            ...
            const url = `${config.baseUrl}/api/v2/post_players${queryString}`;
...
function createPost_PlayersTool(config: APIConfig): Tool {
    return {
        definition: {
            name: "post_players",
        ...
        handler: getPost_PlayersHandler(config)
    };
}
...
export {
    ...
    Tool,
    ToolDefinition,
    getPost_PlayersHandler,
    createPost_PlayersTool
};